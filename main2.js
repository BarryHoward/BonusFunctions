...
function pluck(list, propertyName) {
    var newArray = map(list, function(element){
...
function where(list, properties) {
    var keysArray = Object.keys(properties);
    var newArray = [];
    list.forEach(function(outerElement){
        var check = true;
        keysArray.forEach(function(innerElement){
            if (!(outerElement[innerElement] === properties[innerElement])){
                check = false;
            }
        })
        if (check){
            newArray.push(outerElement);
        }
    })
    return newArray;
}
